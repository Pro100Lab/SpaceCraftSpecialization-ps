<template>
    <v-card class="elevation-0 rounded-0 transparent mx-auto" style="width: 70%;">
    <BreadCrumbs :crumbs="[{text: 'Главная', href:'/', disabled: false}, {text: 'Сравнить товары', href: '', disabled: true}]"/>
        <v-card class="fill-height mt-2 rounded-0">
            <v-card-title>
                Сравнение товаров {{products.length ? `(${products.length})` : ''}}
            </v-card-title>
            <v-divider/>
...
<script>
    import ProductHCard from "../components/products/ProductHCard";
    ...
    export default {
        name: "Compare",
        components: {BreadCrumbs, ProductHCard},
        data: () => {
            return {
                products: []
            }
